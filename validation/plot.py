...
if __name__ == '__main__':
    ...
    for d in dc:
        ...
        for f in fs:
            ...
            max = max(combined['nll'])
            fignll.add_annotation(x=1.0, y=max/1.5-max/5, text=r'$\hat{r} = \text{'+str(combinedata['bf'][0])+'}$', font=dict(family="sans serif", size=16, color=color['combine']))
            fignll.add_annotation(x=1.0, y=max/1.5-2.*max/5, text=r'$\hat{r} = \text{'+str(pyhfdata['bf'][0])+'}$', font=dict(family="sans serif", size=16, color=color['pyhf']))
            print(combinedata['bf'][0], pyhfdata['bf'][0])
            if analyticdata: fignll.add_annotation(x=1.0, y=max/1.5-3.*max/5, text=r'$\hat{r} = \text{'+str(analyticdata['bf'][0])+'}$', font=dict(family="sans serif", size=16, color=color['analytic']))
